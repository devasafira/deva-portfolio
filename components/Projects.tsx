    ...
    > 
      <div id="projects" className="py-5 px-2 md:px-10 lg:px-20">
        <motion.div variants={slideInFromRight(0.5)}>
          <TextGenerateEffect
            words="Projects"
            className="text-4xl text-red-50 font-semibold"
          />
        </motion.div>
          ...
          <div className="flex flex-col lg:flex-row w-full gap-3 justify-around">
            {projectExperience.map((item, i) => (
              <div className="w-auto lg:w-[30vw] h-auto" key={item.id}>
                <BackgroundGradient className="flex flex-col h-auto lg:h-[30.5rem]">
                  <div className="w-auto bg-[#161A31] p-2 rounded-[17px] h-auto lg:h-[30.5rem]">
                    <img
                      src={item.image}
                      alt={item.title}
